...
public final class Renderer{
    ...
    /**
     * A reference to the singleton instance of the {@link com.fifthgen.trafficsim.map.Map} because we need this quite often and don't want to rely on compiler inlining.
     */
	...
	private Renderer(){
		try{
            ...
            URL url = ClassLoader.getSystemResource("blocking.png"); //$NON-NLS-1$
	...
	public synchronized void drawStaticObjects(BufferedImage image){
		...
		if(regions != null && map_.getReadyState()){
            ...
            detailedView = zoom_ > 0.0001;    // only show fast speed streets if zoom is very far away
			for(i = savedRegionMinX; i <= savedRegionMaxX; ++i){
				for(j = savedRegionMinY; j <= savedRegionMaxY; ++j){
					streets = regions[i][j].getStreets();
					for(k = 0; k < streets.length; ++k){
     ...
     * @param forceRenderNow <code>true</code> to force an immediate update regardless of consistency considerations (should only be used by the {@link com.fifthgen.trafficsim.simulation.SimulationMaster})
	 */
