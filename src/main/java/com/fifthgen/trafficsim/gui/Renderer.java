...
public final class Renderer {
    ...
    private Renderer() {
        try {
            ...
            URL url = ClassLoader.getSystemResource("blocking.png");
            ...
            url = ClassLoader.getSystemResource("slippery_road.png");
            tmpImage = ImageIO.read(url);
    ...
    public void drawMovingObjects(Graphics2D g2d) {
        ...
        if (regions != null && map_.getReadyState() && (!simulationRunning_ || doPaintInitializedBySimulation_)) {
                                    ...
                                    if (vehicle.isActive() && vehicle.getX() >= mapMinX_ && vehicle.getX() <= mapMaxX_ && vehicle.getY() >= mapMinY_ && vehicle.getY() <= mapMaxY_) {
                                        ...
                                        if (tmp != null) {
                                            for (int p = 0; p < tmp.getSize(); p++) {
                                                ...
                                                if (tmp.getPenaltySourceVehicle_() != null && tmp.getPenaltySourceVehicle_()[p] != null)
                                                    g2d.drawLine(vehicle.getX(), vehicle.getY(), tmp.getPenaltySourceVehicle_()[p].getX(), tmp.getPenaltySourceVehicle_()[p].getY());
                                            }
                                        }
            ...
            g2d.drawString(FORMATTER.format(timePassed_) + " ms", 5, 10);
    ...
    public synchronized void drawStaticObjects(BufferedImage image) {
        ...
        if (regions != null && map_.getReadyState()) {
            ...
            if (!lastOverdrawn_ && (((panCountX_ == 1 || panCountX_ == -1) && panCountY_ == 0) || ((panCountY_ == 1 || panCountY_ == -1) && panCountX_ == 0))) {
                ...
                g2d.drawImage(image, Math.round(panCountX_ * drawWidth_ / 2), Math.round(panCountY_ * drawHeight_ / 2), null, drawArea_);
                ...
                for (i = regionMinX_; i <= regionMaxX_; ++i) {
                    for (j = regionMinY_; j <= regionMaxY_; ++j) {
                        ...
                        for (int l = 0; l < rsus.length; ++l) {
                            if (highlightCommunication_)
                                g2d.drawOval(rsus[l].getX() - rsus[l].getWifiRadius(), rsus[l].getY() - rsus[l].getWifiRadius(),
                                        rsus[l].getWifiRadius() * 2, rsus[l].getWifiRadius() * 2);
            ...
            while (tmpEventSpot != null) {
                ...
                g2d.drawOval(tmpEventSpot.getX_() - tmpEventSpot.getRadius_(), tmpEventSpot.getY_() - tmpEventSpot.getRadius_(),
                        tmpEventSpot.getRadius_() * 2, tmpEventSpot.getRadius_() * 2);
