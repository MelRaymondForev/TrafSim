					...
					if(oldTime != 0 && Math.abs(actualTime - oldTime) > 1000){
						if(tmpString2.equals("")){
							...
							for(int j = 0; j < dataOne.length-1; j+=2){
                                ...
                                expectedWay2 = (double) (silentPeriodTime / 1000 * Integer.parseInt(logData[streetSpeed])) * tuneTimeValue;
					...
					if(oldTime != 0 && Math.abs(actualTime - oldTime) > 1000){
						if(tmpString2.equals("")){
							...
							for(int j = 0; j < dataOne.length-1; j+=2){
								...
								v1 = x2 - x1;
								v2 = y2 - y1;
                                normalize = (float) (1 / Math.sqrt((v1 * v1) + (v2 * v2)));
...
/*
		...
		*/
        ...
        logAnalyser_.updateInformationArea("Slow " + numberOfVehicles + " " + numberOfCorrectVehicles + " " + numberOfVehiclesLeftTooSoon + " " + ((float) numberOfCorrectVehicles / (float) numberOfVehicles * 100) + " " + (float) ((numberOfCorrectVehicles - numberOfVehiclesLeftTooSoon) * 100) / (numberOfVehicles - numberOfVehiclesLeftTooSoon) + "\n", true);
						...
						if(logData[direction].equals("IN") && logData[nodeID].equals(theNodeID)){
							...
							try {
								...
								while ((line = reader.readLine()) != null) {   
                                    ...
                                    if (logData2[direction].equals("OUT") && logData[nodeID].equals(theNodeID))
                                        savedInMixZone.remove(logData2[steadyID]);
