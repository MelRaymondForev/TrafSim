...
import com.fifthgen.trafficsim.VanetSimStart;
...
public final class VehicleTypeDialog extends JDialog implements ActionListener, MouseListener{
		...
		if ("comboBoxChanged".equals(command)){
			...
			if(chooseVehicleType_.getItemCount() > 0){
                ...
                for (int i = 0; i < chooseVehicleType_.getItemCount(); i++)
                    tmpList.add(chooseVehicleType_.getItemAt(i));
	...
	public void saveType(VehicleType tmpType) {	
        ...
        tmpType.setMinCommDist(Math.round(((Number) minCommDist_.getValue()).intValue() * 100));
        tmpType.setMaxCommDist(Math.round(((Number) maxCommDist_.getValue()).intValue() * 100));
        tmpType.setMinWaittime(Math.round(((Number) minWait_.getValue()).intValue()));
        tmpType.setMaxWaittime(Math.round(((Number) maxWait_.getValue()).intValue()));
        tmpType.setMinBrakingRate(Math.round(((Number) minBraking_.getValue()).intValue()));
        tmpType.setMaxBrakingRate(Math.round(((Number) maxBraking_.getValue()).intValue()));
        tmpType.setMinAccelerationRate(Math.round(((Number) minAcceleration_.getValue()).intValue()));
        tmpType.setMaxAccelerationRate(Math.round(((Number) maxAcceleration_.getValue()).intValue()));
        tmpType.setMinTimeDistance(Math.round(((Number) minTimeDistance_.getValue()).intValue()));
        tmpType.setMaxTimeDistance(Math.round(((Number) maxTimeDistance_.getValue()).intValue()));
        tmpType.setMinPoliteness(Math.round(((Number) minPoliteness_.getValue()).intValue()));
        tmpType.setMaxPoliteness(Math.round(((Number) maxPoliteness_.getValue()).intValue()));
        ...
        tmpType.setVehiclesDeviatingMaxSpeed_(Math.round(((Number) vehiclesDeviatingMaxSpeed_.getValue()).intValue()));
	}
	...
	public void saveQuestions(){
		...
		ArrayList<VehicleType> tmpList = new ArrayList<VehicleType>();
        for (int i = 0; i < chooseVehicleType_.getItemCount(); i++) tmpList.add(chooseVehicleType_.getItemAt(i));
	...
	public void removeEmptyTypes(){
		for(int i = 0;i < chooseVehicleType_.getItemCount(); i++){
            if (chooseVehicleType_.getItemAt(i).getName().equals("")) chooseVehicleType_.removeItemAt(i);
		}
	}
