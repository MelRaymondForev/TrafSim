...
import com.fifthgen.trafficsim.gui.controlpanels.MapSizeDialog;
import com.fifthgen.trafficsim.gui.helpers.MouseClickManager;
import com.fifthgen.trafficsim.localization.Messages;
import com.fifthgen.trafficsim.routing.astar.LookupTableFactory;
...
import com.fifthgen.trafficsim.scenario.events.EventSpot;
...
public final class Map {
                                            ...
                                            if (childtype.equals("name")) {
                                                ...
                                                while (nodeCrsr.getNext() != null) {
                                                    if (nodeCrsr.getLocalName().toLowerCase().equals("x")) {
                                                        ...
                                                        trafficSignal = nodeCrsr.collectDescendantText(false).toLowerCase().equals("true");
                ...
                for (j = 0; j < regionCountY_; ++j) {
                    ...
                    for (k = 0; k < streetsArray.length; ++k) {
                        ...
                        if (street.getMainRegion() == regions_[i][j]) {
                            ...
                            else level1.addElement("Oneway").addCharacters("false");
    ...
    public void addRSU(RSU rsu) {
        int regionX = rsu.getX() / regionWidth_;
            ...
            for (int i = 0; i < regions_.length; i++) {
                for (int j = 0; j < regions_[i].length; j++) {
                    ...
                    for (int k = 0; k < tmpStreets.length; k++) {
                        ...
                        if (((tmpStartNode.getX() < minX || tmpStartNode.getY() < minY) &&
                        ...
                        } else if (tmpStartNode.getY() < minY || tmpStartNode.getY() > maxY ||
                                tmpEndNode.getY() < minY || tmpEndNode.getY() > maxY ||
                                tmpStartNode.getX() < minX || tmpEndNode.getX() < minX ||
                                tmpStartNode.getX() > maxX || tmpEndNode.getX() > maxX) {
                            ...
                            if (tmpEndNode.getY() < minY || tmpEndNode.getY() > maxY || tmpEndNode.getX() < minX ||
                                    tmpEndNode.getX() > maxX) {
                                tmp = tmpStartNode;
                            ...
                            if (tmpStartNode.getX() < minX || tmpEndNode.getX() < minX || tmpStartNode.getX() > maxX ||
                                    tmpEndNode.getX() > maxX) {
                                tmpStartNode.setY(trimStreet(tmpEndNode.getY(), tmpEndNode.getX(), tmpStartNode.getY(),
                                        tmpStartNode.getX(), tmpBorder));
                                tmpStartNode.setX(tmpBorder);
                            } else {
                                tmpStartNode.setX(trimStreet(tmpEndNode.getX(), tmpEndNode.getY(),
                                        tmpStartNode.getX(), tmpStartNode.getY(), tmpBorder));
    ...
    public void saveReloadMap() {
        Application.getMainControlPanel().changeFileChooser(false, true, false);
        final JFileChooser filechooser = Application.getMainControlPanel().getFileChooser();
        int returnVal = filechooser.showSaveDialog(Application.getMainFrame());
    ...
    public void clearTrafficLights() {
        for (int i = 0; i < regionCountX_; ++i)
            for (int j = 0; j < regionCountY_; ++j) regions_[i][j].clearTrafficLights();
    }
