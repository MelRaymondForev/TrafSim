...
public final class StartBlocking extends Event {
    ...
    public StartBlocking(int time, int x, int y, int direction, int lanes, boolean isFake, String penaltyType) throws ParseException {
        ...
        } else throw new ParseException(Messages.getString("StartBlocking.snappingFailed"), 0);
    }

    public String getText() {
        String tmp;
        if (affectedDirection_ == 0) tmp = Messages.getString("StartBlocking.both");
        else if (affectedDirection_ == 1) tmp = Messages.getString("StartBlocking.fromStartNode");
        else tmp = Messages.getString("StartBlocking.fromEndNode");
